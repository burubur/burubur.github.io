...
export default function Portfolio({ pageContext = {} }) {
  ...
  return (
    <Layout>
      <div
        className={`content portfolio-page${
          isViewMode ? " portfolio-page-view-mode" : ""
        }`}
      >
        <div className="portfolio-header">
          <h1 className="portfolio-title">{summary.title}</h1>
          <p className="portfolio-summary">{summary.description}</p>
        </div>
        {isViewMode ? (
          ...
          <div
            className="experience-timeline portfolio-list"
...
function ProjectCard({ project, onOpen }) {
  const { year, name, brief, url, type, image, imageAlt, techStack } = project
  ...
  return (
    <div
      className={`experience-card portfolio-card${
        image ? " portfolio-card-with-image" : ""
      }`}
      style={{
        marginBottom: "1.5rem",
        padding: "1.5rem",
...
function ProjectView({ project, onBack }) {
  const { name, brief, image, imageAlt, techStack, type, year } = project
